...
The BazCoin Core daemon can be configured to act as a trusted "border
router", implementing the bazcoin wire protocol and relay, making
...
The ZeroMQ feature in BazCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bazcoind:
...
For instance:
    ...
    $ bazcoind -zmqpubhashtx=tcp://127.0.0.1:235575 \
               -zmqpubhashtx=tcp://192.168.1.2:235575 \
               -zmqpubrawtx=ipc:///tmp/bazcoind.tx.raw \
...
These options can also be provided in bazcoin.conf.
...
From the perspective of bazcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bazcoind directly. Furthermore, no information is
...
using. BazCoind appends an up-counting sequence number to each
