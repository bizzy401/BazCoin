...
"""Test bazcoind with different proxy configuration.

Test plan:
- Start bazcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bazcoind side:
...
class ProxyTest(BazCoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: bazcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
        ...
        if test_onion:
            addr = "bazcoinostk4e4re.onion:35576"
            self.log.debug("Test: outgoing onion connection through node for address {}".format(addr))
            node.addnode(addr, "onetry")
            cmd = proxies[2].queue.get()
            assert isinstance(cmd, Socks5Command)
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"bazcoinostk4e4re.onion")
            assert_equal(cmd.port, 35576)
            if not auth:
                assert_equal(cmd.username, None)
                assert_equal(cmd.password, None)
            rv.append(cmd)
