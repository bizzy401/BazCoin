...
const char * const BAZCOIN_CONF_FILENAME = "bazcoin.conf";
const char * const BAZCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bazcoin/bazcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bazcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BAZCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BazCoin
    // macOS: ~/Library/Application Support/BazCoin
    // Unix-like: ~/.bazcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BazCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BazCoin";
#else
    // Unix-like
    return pathRet / ".bazcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BAZCOIN_CONF_FILENAME);
