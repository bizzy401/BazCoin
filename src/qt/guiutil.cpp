...
#include <qt/bazcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BazCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BazCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new BazCoinAddressCheckValidator(parent));
}

bool parseBazCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bazcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("bazcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BazCoinUnits::parse(BazCoinUnits::BAZ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBazCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBazCoinURI(uriInstance, out);
}

QString formatBazCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bazcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BazCoinUnits::format(BazCoinUnits::BAZ, info.amount, false, BazCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBazCoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BAZCOIN_CONF_FILENAME));
    ...
    /* Open bazcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BazCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BazCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BazCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BazCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bazcoin.desktop";
    return GetAutostartDir() / strprintf("bazcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bazcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BazCoin\n";
        else
            optionFile << strprintf("Name=BazCoin (%s)\n", chain);
