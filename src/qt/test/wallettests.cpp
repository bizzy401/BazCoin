...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<BazCoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_bazcoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_bazcoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_bazcoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = BazCoinUnits::formatWithUnit(unit, balance, false, BazCoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    BazCoinAmountField* amountInput = receiveCoinsDialog.findChild<BazCoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_bazcoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
